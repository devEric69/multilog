...
uses
  StrUtils, LCLIntf, LCLType, fAbout{$If defined(DEBUG)},LazLoggerBase{$EndIf};
...
var
  ...
  iNbrItemInSet, ieItemInSet: Integer;
{$If defined(DEBUG)}
...
begin
  ...
  with TSimpleIPCServer(Sender).MsgData do begin
    ...
    ReadBuffer(SizeOfText,SizeOf(Integer)); // read FCurrentMsg.sMsgText[0]
    if SizeOfText>0 then begin																						{ if there's a text to read? }
    	SetLength(FCurrentMsg.sMsgText,SizeOfText);
    	ReadBuffer(FCurrentMsg.sMsgText[1],SizeOfText);
    end
    else
...
{$EndIf;}
    ...
    if SizeOfData > 0 then begin
      ...
      FCurrentMsg.pData.CopyFrom(TSimpleIPCServer(Sender).MsgData,SizeOfData);
    end
    else
